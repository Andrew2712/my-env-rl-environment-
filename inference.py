...
def _clamp(value: float) -> float:
    """Clamp reward to strictly (0, 1) exclusive — validator rejects 0.0 and 1.0."""
...
def _fmt(value: float) -> str:
    ...
    Format reward for display:
      0.001 (clamped from 0.0) → '0'
      0.999 (clamped from 1.0) → '1'
      anything else            → e.g. '0.60'
    """
    if value <= 0.001:
        return "0"
    if value >= 0.999:
...
- Example: {"password": "Xt9#pq", "reasoning": "Testing digit and symbol rules"}
"""
...
def build_prompt(obs: dict, step: int, episode_history: list) -> str:
    ...
    elif abs(last_r - 0.8) < 0.02:
        hint = "ONE rule left. Try symbol (@#$%), length, or first character."
    elif abs(last_r - 0.6) < 0.02:
        hint = "TWO rules left. Fix one dimension at a time."
    elif abs(last_r - 0.4) < 0.02:
        hint = "THREE rules left. Start with a broader password."
    elif last_r < 0.02:
        hint = "No rules passing yet. Try a password with uppercase, lowercase, digit, and symbol."
    else:
        hint = f"{rules_left} rules left. Keep isolating changes from your best password."
...
def get_agent_action(obs: dict, step: int, episode_history: list, llm) -> tuple[str, str]:
    ...
    if llm is not None:
        ...
        for attempt in range(3):
            try:
                ...
                if not pw:
                    last_llm_error = "empty password"
                    continue

                if pw in submitted:
                    last_llm_error = f"duplicate: {pw}"
                    current_obs = dict(current_obs)
                    current_obs["_duplicate_warning"] = (
                        f"'{pw}' already tried — pick a completely DIFFERENT password."
                    )
                    continue
        ...
        print(f"[WARN] LLM failed ({last_llm_error}). Using fallback.", flush=True)

    # ── Smart fallback ─────────────────────────────────────────────────────────
    best_entry = max(episode_history, key=lambda h: h["reward"]) if episode_history else None
    best_pw    = best_entry["password"] if best_entry else "Xt9#pq"
    symbols    = ["@", "#", "$", "%"]
    lengths    = [5, 6, 7, 8]
    fallback_candidates = []
    ...
    fallback_candidates += [
        "Xt9#pq", "Xt9@pq", "Xt9$pq", "Xt9%pq",
        "Mz3@kl", "Mz3#kl", "Pq7$rs", "Pq7%rs",
            ...
            for step in range(1, MAX_STEPS + 1):
                ...
                try:
                    obs_obj, reward_obj, done, info = env.step(
                        person_id=person_id, password=password
                    )
                    obs        = obs_obj.model_dump()
                    raw_reward = (
                        reward_obj.value
                ...
                print(
                    ...
                    f"reward={_fmt(clamped)} done={done_str} error={error_msg}",
                    flush=True,
                )

                if done:
                    success = (clamped >= 0.999)
                    break
...
if __name__ == "__main__":
    ...
    for task in TASKS:
        run_episode(
            task=task,
            person_id=f"baseline_agent_{task}_{run_id}",
            llm=llm,
        )
        print(flush=True)